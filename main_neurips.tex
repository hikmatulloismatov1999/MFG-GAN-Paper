...
momentum directly.  We study the weakly non-separable class
\(H(x,\rho,p)=H_0(x,p)-f_0(x,\rho)+\eps R(x,\rho,p)\) through an outer
fixed-point iteration that freezes the density in the residual term.  Each
outer step is then a separable MFG subproblem and can be passed to a GAN-based
inner solver.  Under Lasry--Lions monotonicity, strong convexity of the base
Hamiltonian, bounded densities, and bounded momentum, we prove that the exact
outer map is an \(L^2\) contraction with explicit rate
\(\kappa=\eps L_R/\lambda\).  This gives an induced convergence bound in
\(\W_2\).  We prove sharpness for a linearized ergodic family on the torus and
verify the predicted phase transition in deterministic linearized experiments.
\end{abstract}
