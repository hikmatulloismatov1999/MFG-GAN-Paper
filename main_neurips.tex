...
pipeline \citep{achdou2010mean,lavigne2023policy,hu2024recent}.  Our result is
therefore best understood as an outer contraction principle for turning a weak
...
\paragraph{Scope.}
The analysis does not prove convergence of stochastic neural-network training.
Instead, it proves that if the separable inner problem is solved exactly, or up
to a controlled error, then the non-separable outer iteration contracts.  This
separation is useful because optimization guarantees for the inner GAN solver
can be improved independently of the outer fixed-point analysis.
...
This is the quantity controlled by \Cref{thm:l2_contraction_neurips}, so the
experiments test the theorem's rate and failure threshold directly.
...
and high-dimensional benchmarks are natural next steps.  Important limitations
