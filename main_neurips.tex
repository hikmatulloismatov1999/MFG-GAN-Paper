...
\(3\eps\rho_{\max}<c_0\rho_{\min}\); our traffic experiment studies the
corresponding linearized rate around the uniform no-drift equilibrium.
...
\]
The domination condition \(3\eps L_{R,p}\rho_{\max}<c_0\rho_{\min}\) absorbs
\(\eps A_{R,p}\) into the negative base contribution, leaving only the
density-variation term.
\end{proof}
...
contraction estimate.  The case \(\delta\rho=0\) is trivial.  Banach's
