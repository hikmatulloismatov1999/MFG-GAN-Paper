...
\title{A Fixed-Point Framework for GAN-Based Mean Field Games\\
...
interacts directly with momentum or control, including traffic-flow and
congestion-type MFGs.  We propose a fixed-point framework that extends
GAN-based MFG solvers to weakly non-separable Hamiltonians of the form
...
to a separable MFG subproblem that can be solved using existing GAN-based inner
solvers.  Under Lasry--Lions monotonicity, strong convexity of the base
Hamiltonian, and boundedness assumptions on density and momentum, we prove
that the exact outer map is an \(L^2\) contraction with rate proportional to
\(\eps\).  This yields convergence of the proposed outer scheme and identifies
an explicit weak-coupling threshold.  We further show that the threshold is
sharp for a linearized ergodic model on the torus.  Numerical experiments
validate the predicted phase transition and support the proposed extension
beyond the separable setting.
\end{abstract}
